patches-own[payoff strategy visited]
globals[b R P S level]

to setup
  clear-all
  setup-patches
  set b 2.0 ;T. It should be between 1.8 and 2.0 in order to achieve the kaleidoscopic pattern.
  set R 1.0
  set P 0.01
  set S 0.0
  set level 0
...
to update-patch  ;run only once at the beginning
  ifelse strategy = 0
  [set pcolor red]
  [set pcolor blue]
end
...
to calculate-payoff
  ...
  [
    foreach sort neighbors[
      [n] -> ifelse ([strategy] of n = 0)
      [set payoff payoff + S set payoff precision payoff 2]
      [set payoff payoff + R set payoff precision payoff 2]
    ]
  ]
  [
    foreach sort neighbors[
      [n] -> ifelse ([strategy] of n = 0)
      [set payoff payoff + P set payoff precision payoff 2]
      [set payoff payoff + b set payoff precision payoff 2]
    ]
  ]
end
...
to update-strategy
  ...
  ask max-one-of neighbors [payoff] [
    if max-pay < payoff [
      set max-pay payoff
      set new-strategy strategy
    ]
  ]
  if max-pay != -1 [
    if new-strategy = 1 and strategy = 1
    [set pcolor blue]
    if new-strategy = 0 and strategy = 0
    [set pcolor red]
    if new-strategy = 1 and strategy = 0
    [set pcolor green]
    if new-strategy = 0 and strategy = 1
    [set pcolor yellow]
...
This code was written by Zineb ELHAMER, based on the model description in the paper: 'Evolutionary games and spatial chaos', Martin A. Nowak and Robert M. May. Nature 359(6398):826-829, 1992.
@#$#@#$#@
default
true
0
